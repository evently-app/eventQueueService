var scraper = require("../scrapers/eventbrite.js");
...
//checks scraper.tag(description) function.
describe("tag", function() {
    it("null string", function() {
        assert.equal(scraper.tag(null).length, 0);
    });
    it("empty string", function() {
        assert.equal(scraper.tag("").length, 0);
    });
    it("some tags", function() {
        assert.deepEqual(scraper.tag("free show"), ["Free", "Performance"]);
    });
});
